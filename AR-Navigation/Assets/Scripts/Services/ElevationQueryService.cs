...
namespace Assets.Scripts.Services
{
    public class ElevationQueryService
    {
        ...
        public async Task<OpenTopoDataResponse> MakeOpenTopoDataQuery(List<Vector2> locations, ElevationAPI elevationApi)
        {
            if (elevationApi != ElevationAPI.OPEN_TOPO_DATA_EUDEM && elevationApi != ElevationAPI.OPEN_TOPO_DATA_ASTER)
                throw new Exception($"Wrong Dataset requested: Expecting OpenTopoData, got {elevationApi}");

            string locationsString = ConstructLocationsListStringFromVector2Array(locations);

            string dataset = elevationApi == ElevationAPI.OPEN_TOPO_DATA_EUDEM ? OPENTOPODATA_DATASET_EUDEM : OPENTOPODATA_DATASET_ASTER;
